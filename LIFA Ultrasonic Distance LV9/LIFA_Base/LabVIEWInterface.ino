  ...
  if(command[0] == 0xFF && checksum_Test(command) == 0)
  {
    ...
    *********************************************************************************/
    case 0x34:  // IR Transmit  
      ...
      break;
	case 0xF4:
		triggerpin = command[2];  // Trigger Pin
		signalpin = command[3];  // Signal Pin
		triglen = command[4];  // Trigger Pulse Length (us)
		...
		pulse_length = pulseIn(signalpin, HIGH, 250000); // 0.25 second timeout for pulse start
		
		// Send result to LabVIEW
		Serial.write((pulse_length >> 24) & 0xFF);
		Serial.write((pulse_length >> 16) & 0xFF);
		Serial.write((pulse_length >>  8) & 0xFF);
		Serial.write(pulse_length & 0xFF);
